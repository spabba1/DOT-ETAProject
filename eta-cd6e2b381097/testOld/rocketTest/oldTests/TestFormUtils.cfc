<cfcomponent extends="testbox.system.BaseSpec">
    ...
    <cffunction name="setUp" output="false" access="public" returntype="void" hint="">
        ...
        <cfset util = CreateObject('component', 'eta.model.services.FormUtilities').init( true ) />
        ...
        <cffile action="read" file="#getDirectoryFromPath( getCurrentTemplatePath() )#result.xml" variable="resultWDDX" />
    ...
    <cffunction name="testCollectionCreation" access="public" returntype="void">
        ...
        <cfloop list="#formData.formCollectionsList#" index="local.thisColl" delimiters=",">
            <cfif isArray(formData[local.thisColl])>
                <cfset $assert.isTrue(arrayCompare(formData[local.thisColl], result[local.thisColl]), 'Collection #local.thisColl# does not match expected results.') />
            <cfelseif isStruct(formData[local.thisColl])>
                <cfset $assert.isTrue(structCompare(formData[local.thisColl], result[local.thisColl]), 'Collection #local.thisColl# does not match expected results.') />
            </cfif>
        </cfloop>
    </cffunction>
